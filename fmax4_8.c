...
//if(z&S,1)printf("%d:%d (%2x,%2x), %6d %6d (%2x,%2x)\n",n,d,x,y,v,m,K,L);
        if(z&S&&K-I)                           /* move pending: check legal*/
        {if(v+I&&x==K&y==L)                    /*   if move found          */
         {Q=-e-i;O=F;LL=L;
          if(b[y]-u&15)b[y]-=PromPiece,J++;    /* under-promotions, invalidate hash */
...
int LoadGame(char *name)
{
        ...
        f = fopen("fmax.ini", "r");
        if(f==NULL)
        {   printf("telluser piece-desription file 'fmax.ini' not found\n");
            exit(0);
        }
...
int main(int argc, char **argv)
{
	...
	signal(SIGINT, SIG_IGN);
        printf("tellics say     fairy-Max 4.8 (E)\n");
		...
		if (!strcmp(command, "xboard"))
			continue;
                if (!strcmp(command, "protover")) {
                        printf("feature myname=\"Fairy-Max 4.8E\"\n");
		...
		if (!strcmp(command, "variant")) {
                        ...
                        while(fgets(line, 256, stdin)) {
                                ...
                                if( m >= 'A' && m <= 'Z' && piecetype[m&31]
                                    ...
                                    && line[2] >= '1' && line[2] <= '0'+BH) {
                                        ...
                                        p = 4; /* Knight code */
                                        switch(defaultchar[piecetype[line[0]&31]])
                                        {
                                        case 'P':
                                            if(color==WHITE)
			...
			continue;
                ...
                if(line[4] == '\n') line[4] = piecename[7];
                switch(defaultchar[piecetype[line[4]&31]]+32) {
                    default:
                    case 'q':  PromPiece=0; break;
                    case 'r':  PromPiece=1; break;
                    case 'b':  PromPiece=2; break;
                    case 'n':  PromPiece=3; break;
                    case 'e':  PromPiece=-1; break;
                    case 'w':  PromPiece=-2; break;
                    case 'f':  PromPiece=-3; break;
                    case 'm':  PromPiece=-4; break;
                    case 'a':  PromPiece=-5; break;
                    case 'c':  PromPiece=-6; break;
